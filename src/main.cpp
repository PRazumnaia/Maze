...
public:
  ...
  void update(float time) {
    ...
    if (dy < 0)
      sprite.setTextureRect(IntRect(2 + 32 * int(currentFrame), 4, 29, 29));
    if (dy > 0)
      sprite.setTextureRect(IntRect(2 + 32 * int(currentFrame), 68, 29, 29));
    if (dx < 0)
      sprite.setTextureRect(IntRect(2 + 32 * int(currentFrame), 36, 29, 29));
    if (dx > 0)
      sprite.setTextureRect(IntRect(2 + 32 * int(currentFrame), 100, 29, 29));
...
int main() {
  ...
  while (window.isOpen()) {
    ...
    if (p.count == 40) {
      ...
      window.clear(Color::Black);
      window.draw(yw);
      text.setString(L"Вы собрали монеток: " + gameMoneyString.str() +
    ...
    if (timer(gameTime, second) == 0) {
      ...
      window.clear(Color::Black);
      window.draw(go);
      text.setString(L"Вы успели собрать монеток: " + gameMoneyString.str() +
                     L"\nНажмите Esc, чтобы выйти.");
